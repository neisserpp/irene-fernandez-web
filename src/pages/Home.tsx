...
function Hero() {
  return (
    <section className="grain slash-panel wave-foot-bone relative overflow-hidden bg-night-900 pt-32 pb-24 text-bone-100 md:pt-40 md:pb-32 lg:pt-44 lg:pb-36">
      <div
        aria-hidden
        className="absolute inset-0 bg-[radial-gradient(circle_at_82%_28%,rgba(31,79,70,0.22),transparent_34%),radial-gradient(circle_at_8%_80%,rgba(115,61,78,0.20),transparent_30%)]"
      />
      <div
        aria-hidden
        className="absolute right-0 top-0 hidden h-full w-[35%] border-l border-white/10 lg:block"
      />
        ...
        <div className="grid items-end gap-12 lg:grid-cols-12 lg:gap-10">
          <div className="lg:col-span-7">
            <Reveal direction="left">
              ...
              <h1 className="mt-7 max-w-5xl text-[2.65rem] leading-[1.03] tracking-[-0.025em] sm:text-5xl md:text-6xl lg:text-[5.25rem]">
                ...
                <span className="italic font-semibold text-petrol-100">
          ...
          <Reveal direction="right" delay={160} className="lg:col-span-5 lg:pl-8">
            <div className="relative mx-auto max-w-md lg:max-w-none">
              <div
                aria-hidden
                className="absolute -right-4 -top-4 h-24 w-24 rounded-full border border-terra-500/60"
              />
              <ImageFrame
              ...
              />
              <div className="absolute -bottom-6 -left-5 z-20 max-w-[230px] rounded-[20px_0_20px_0] border border-white/15 bg-night-900/95 p-5 backdrop-blur-md">
                <p className="eyebrow text-terra-500">
                  Criterio · liderazgo · transformación
                </p>
                <p className="mt-3 font-display text-lg leading-snug font-semibold text-bone-100">
      ...
      <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {authority.map((item, i) => (
          <Reveal key={item.label} delay={i * 60}>
            <article className="group magnetic-card h-full rounded-[20px] border border-bone-300/80 bg-bone-50 p-7 md:p-9">
              <p className="number-mark font-display text-5xl leading-none text-petrol-600 md:text-6xl">
                {item.figure}
              </p>
              <p className="mt-5 font-display text-lg leading-snug font-semibold text-night-900">
                {item.label}
              </p>
    ...
    <Section tone="night" className="grain wave-foot">
      <div className="grid gap-14 lg:grid-cols-12 lg:gap-20">
        <Reveal direction="left" className="lg:col-span-4">
          ...
          <Link
            to="/vision"
            className="group mt-8 inline-flex items-center gap-2 border-b border-terra-500 pb-1 text-xs font-semibold tracking-[0.14em] uppercase transition-colors hover:text-terra-500"
          >
      ...
      <div className="mt-14 grid gap-4 md:grid-cols-3">
        {workAreas.map((area, i) => (
          <Reveal key={area.title} delay={i * 80}>
            <article className="group magnetic-card relative h-full overflow-hidden rounded-[20px] border border-bone-200 bg-bone-50 p-8 md:p-10">
              <span
                aria-hidden
                className="absolute right-0 top-0 h-24 w-24 -translate-y-1/2 translate-x-1/2 rounded-full bg-terra-100 transition-transform duration-500 group-hover:scale-150"
              />
              <div className="relative">
                ...
                <span className="mt-8 inline-flex items-center gap-2 text-xs font-semibold tracking-[0.14em] text-petrol-600 uppercase">
      ...
      <div className="mt-14 space-y-3">
        {cases.map((item, i) => (
          <Reveal key={item.title} delay={i * 60}>
            <details className="group overflow-hidden rounded-[20px] border border-bone-300 bg-bone-50 transition-colors open:bg-white">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-6 p-7 md:p-9 [&::-webkit-details-marker]:hidden">
                <div className="flex items-start gap-5">
                  ...
                  <h3 className="max-w-3xl font-display text-xl leading-snug md:text-2xl">
                    {item.title}
                  </h3>
                </div>
    ...
    <Section>
      ...
      <div className="mt-14 grid gap-6 lg:grid-cols-12">
        <Reveal direction="left" className="lg:col-span-7">
          <article className="magnetic-card flex h-full flex-col rounded-[20px] border border-bone-200 bg-bone-100 p-8 md:p-10">
            ...
            <div className="mt-10 border-t border-bone-300 pt-7">
              <Link
                to="/ideas"
                className="group inline-flex items-center gap-2 text-xs font-semibold tracking-[0.14em] text-petrol-600 uppercase transition-colors hover:text-terra-600"
              >
        ...
        <Reveal direction="right" className="lg:col-span-5">
            ...
            {rest.map((item) => (
              <li key={item.title} className="group py-6">
                <Link to="/ideas" className="block">
                  ...
                  <h4 className="mt-3 text-xl leading-snug transition-colors group-hover:text-petrol-600">
                    {item.title}
                  </h4>
          ...
          <Link
            to="/ideas"
            className="group mt-8 inline-flex items-center gap-2 border-b border-terra-500 pb-1 text-xs font-semibold tracking-[0.14em] uppercase transition-colors hover:text-terra-600"
          >
    ...
    <Section tone="night" className="grain">
      <div className="mx-auto max-w-5xl text-center">
        <Reveal>
          ...
          <h2 className="mt-6 text-3xl leading-tight md:text-5xl md:leading-[1.15]">
            {closing.quote}
          </h2>
        </Reveal>
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {closing.ctas.map((cta, i) => (
            <Reveal key={cta.label} delay={i * 70}>
              <Link
                to={cta.to}
                className="group flex h-full flex-col justify-between gap-8 rounded-[20px] border border-white/10 bg-night-800/60 p-7 text-left transition-colors hover:bg-night-800 md:p-8"
              >
                <span className="font-display text-xl leading-snug font-semibold text-bone-100">
                  {cta.label}
                </span>
