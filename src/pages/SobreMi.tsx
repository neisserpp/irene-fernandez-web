    ...
    <>
      ...
      <Section>
        <div className="grid items-start gap-12 lg:grid-cols-12">
          <Reveal direction="left" className="lg:sticky lg:top-28 lg:col-span-4">
            <ImageFrame slot={imageSlots.portrait} aspect="portrait" />
          </Reveal>
          <div className="space-y-14 lg:col-span-8">
            {blocks.map((block, i) => (
              <Reveal key={block.number} delay={i * 50}>
                <article className="grid gap-8 border-t border-bone-200 pt-10 lg:grid-cols-12">
                  <div className="lg:col-span-4">
                    <p className="eyebrow text-terra-600">{block.number}</p>
                    <h2 className="mt-4 text-2xl leading-snug md:text-3xl">{block.title}</h2>
                  </div>
      ...
      <Section tone="night" className="grain wave-foot">
        <div className="mx-auto max-w-3xl text-center">
          <Quote tone="night">
            Tiene experiencia técnica, ha construido empresa, posee criterio propio y sabe
            liderar personas.
          </Quote>
          <p className="mt-8 text-sm text-bone-300">
            Si al terminar de leer esta página piensas esto, la web está haciendo su trabajo.
          </p>
        </div>
      </Section>
