...
export default function Contacto() {
  ...
  return (
    <>
      <header className="grain slash-panel wave-foot relative overflow-hidden bg-night-900 pt-32 pb-24 text-bone-100 md:pt-40 md:pb-28 lg:pt-44">
          ...
          <Reveal direction="left">
            <div className="max-w-4xl">
              ...
              <p className="mt-8 text-lg leading-relaxed text-bone-300">
                Prefiero una conversación honesta a un formulario frío. Si te resulta más
        ...
        <div className="grid gap-14 lg:grid-cols-12">
                ...
                {contactReasons.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={active}
                      className={`flex w-full items-center justify-between gap-6 rounded-[16px] px-6 py-5 text-left transition-colors ${
                        active
                          ? "bg-night-900 text-bone-50"
                          : "bg-bone-50 text-night-900 hover:bg-bone-100"
                      }`}
                    >
                      <span className="font-display text-xl leading-snug font-semibold">
                        {option.label}
                      </span>
                      <span
            ...
            {reason ? (
              <Reveal direction="up">
                ...
                >
                  ...
                  <div className="mt-8">
                    <label htmlFor="mensaje" className="eyebrow block text-ink-500">
                  ...
                  <button
                    type="submit"
                    className="mt-8 inline-flex items-center gap-2 rounded-full bg-petrol-600 px-7 py-4 text-xs font-semibold tracking-[0.14em] text-bone-50 uppercase transition-colors hover:bg-petrol-700"
                  >
                    Enviar propuesta <ArrowRight />
                  </button>

                  {sent ? (
                    <p className="mt-6 rounded-r-[16px] border-l-4 border-petrol-500 bg-bone-100 px-5 py-4 text-sm text-ink-900">
          ...
          <aside className="lg:col-span-5">
            <Reveal direction="right">
              <ImageFrame
                slot={imageSlots.portrait}
                aspect="portrait"
                className="mb-8 max-h-[520px]"
              />
            </Reveal>
            <div className="magnetic-card rounded-[20px] border border-bone-200 bg-bone-100 p-8 md:p-10">
              ...
              <ul className="mt-6 divide-y divide-bone-300 border-t border-bone-300">
                {contactReasons.map((option) => (
                  <li key={option.id} className="py-4">
                    <p className="font-display text-lg leading-snug font-semibold text-night-900">
                      {option.label}
                    </p>
            ...
            <div className="mt-8 rounded-[20px] bg-night-900 p-8 text-bone-100 md:p-10">
              ...
              <p className="mt-8 text-sm leading-relaxed text-bone-300">
                Para proyectos de transformación y SAP, EssensUp responde por la ejecución:{" "}
                <a
                  href={site.essensup}
                  target="_blank"
                  rel="noreferrer"
